        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {tables.map(table => (
              <div
                key={table.id}
                className={`bg-zinc-900/40 border border-zinc-800 rounded-2xl p-4 space-y-3 group transition hover:border-zinc-700 ${!table.is_active ? 'opacity-40' : ''}`}
              >
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 bg-zinc-800 hover:bg-amber-500/10 hover:text-amber-400 hover:border-amber-500/20 border border-zinc-700 text-zinc-400 text-[10px] font-bold rounded-xl transition"
                >
                  <QrCode className="w-3 h-3" /> QR Göster
                </button>
              </div>
            ))}
          </div>
